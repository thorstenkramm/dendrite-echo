...
- [Shadcn UI for Vue](https://www.shadcn-vue.com/) as the component library
...
The development follows the [official vue styleguide](https://vuejs.org/style-guide/).

## Preface

This project is developed my experienced senior software developers with a strong focus on future maintenance.
Code is written for humans. Understandability and a comprehensive structure are crucial.

## Testing and Linting

After each significant change and after a task has been completed, run the tests and linters as follows.
Fix the discovered issues and run the test or linter again until it passes without errors.
...
Try to fix finding with `npm audit fix`. Never run `npm audit fix --force` as this can lead to unpredictable results.
...
Test coverage must always be 75% or above. To get test coverage:
...
Test and make sure the projects builds without errors.
...
This project uses [Shadcn UI for Vue](https://www.shadcn-vue.com/) as its component library. Shadcn is not a
traditional component library - it provides copy-paste components that you own and can customize.

### How Shadcn Works

Components are installed into `src/components/ui/` and become part of your codebase. The configuration is
stored in `components.json`. Key differences from traditional libraries:

- Components are copied into your project, not imported from `node_modules`
- You own the code and can modify components as needed
- Updates are opt-in by re-running the add command
...
Import components from their ui folder:
...
Many components support variants via props. Use the component's props for styling variations:
...
1. Edit the component file in `src/components/ui/<component>/`
2. Modify styles, add props, or change behavior
3. Changes apply immediately throughout your app

### Best Practices

1. **Always use Shadcn components** instead of native HTML elements with inline Tailwind classes for
   consistency.
2. **Don't edit `src/components/ui/` components** unless you need project-specific customizations. This makes
   future updates easier.
3. **Check available components first** before building custom UI. Shadcn likely has what you need.
4. **Use the `cn()` utility** from `@/lib/utils` when combining Tailwind classes:
...
Run `npx shadcn-vue@latest add` to see the full list. Common components include:
...
This project uses a layered approach for API communication with the dendrite-pulse backend:
